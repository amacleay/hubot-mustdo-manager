...
class MustDoClient
  ...
  task_manager_action: (command) ->
    ...
    if commandParts?
      [date, action, subcommand] = commandParts

      if actionDispatch[action]
        try
          actionDispatch[action] subcommand, date
        catch e
